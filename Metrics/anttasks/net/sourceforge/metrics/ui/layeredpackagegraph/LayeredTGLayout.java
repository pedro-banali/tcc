...
public class LayeredTGLayout implements TGLayout {
            ...
            public void forEachNodePair(Node n1, Node n2) {
                if (LayeredPackageTableView.getLayer(n1.getID()) == LayeredPackageTableView
                    ...
                    double labelWidth = ((n1.getID().length() + n2.getID()
                            .length()) * 4);
                    if (len == 0) {
                        dx = n1.dx - n2.dx > 0 ? getDesiredSeparation(labelWidth)
                                : -getDesiredSeparation(labelWidth);
    ...
    /**
     * Move the whole set of nodes towords origo (0,0)
     */
    ...
    public void run() {
        ...
        while (relaxer == me) {
            relax();
            try {
                // Delay to wait for the prior repaint command to finish.
                Thread.sleep(20);
    ...
    public double getStartPositionY(int layer) {
        return layer * -DESIRED_SEPARATION;
    }
