...
/**
 * Panel to display the layered depency graph.
 */
public class LayeredPackageGraphPanel extends GLPanel {
    ...
    public void createDependencies(List layers) throws TGException {
        ...
        if (LayeredPackageTableView.getCurrentPackage() != null) {
            center = tgPanel.findNode(LayeredPackageTableView
                    .getCurrentPackage());
        }
    ...
    private StrongComponent[] calculateCycles(Map efferent) {
        ...
        vgraph = (Vertex[]) graph.toArray(new Vertex[0]);
    ...
    private Node addNode(String id, int layer, int layerWidth)
        ...
        if (n == null) {
            ...
            n.x = layerWidth * 5 * Math.pow(-1, layerWidth);
            n.y = ((LayeredTGLayout) tgPanel.tgLayout).getStartPositionY(layer);
            tgPanel.addNode(n);
        }
        return n;
    }
